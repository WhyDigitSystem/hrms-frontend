...
export const CompoOff = () => {
  ...
  const handleSave = async () => {
    ...
    // Map errors back to original table
    const mergedErrors = leaveTypeTable.map(row => {
      const index = editableRows.findIndex(r => r.id === row.id);
      return row.disabled ? { compoOff: '', assignedBy: '', description: '', notify: '' } :
    ...
    try {
      ...
      if (result.status === true) {
        ...
        await sendEmailNotification(finalPayload);
        handleClear();
        getAllCompoOff(); // Reload and disable all data
      } else {
        showToast('error', result.paramObjectsMap?.errorMessage || 'Compo Off creation failed');
      }
    } catch (err) {
  ...
  const getAllCompoOff = async () => {
    try {
      ...
      if (
        response.status &&
        response.data?.paramObjectsMap?.compensatoryOffVO &&
        Array.isArray(response.data.paramObjectsMap.compensatoryOffVO)
      ) {
        const fetchedRows = response.data.paramObjectsMap.compensatoryOffVO.map((item, index) => ({
        ...
        console.log('Mapped rows:', fetchedRows);
